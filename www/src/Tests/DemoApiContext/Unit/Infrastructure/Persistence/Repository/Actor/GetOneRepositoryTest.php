...
/**
 * Class GetOneRepositoryTest
 *
 * @category Tests\DemoApiContext\Unit
 * @package Infrastructure
 * @subpackage Persistence\Repository\Actor
 *
 * @coversDefaultClass GetOneRepository
 * @group unit
 * @requires PHP 7.0
 *
 * @license MIT
 */
class GetOneRepositoryTest extends TestCase
{
    /** @var GetOneRepository $repository */
    ...
    public function setUp(): void
    {
        ...
        $this->repository = new GetOneRepository($this->em, Actor::class);
    ...
    {
        Phake::when($this->query)->getResult()->thenReturn([Phake::mock(Actor::class)]);
        Phake::when($this->qb)->getQuery()->thenReturn($this->query);
        $result = $this->repository->execute($this->object);
        Phake::verify($this->em, Phake::times(1))->createQueryBuilder(Phake::anyParameters());
        Phake::verify($this->qb, Phake::times(1))->getQuery(Phake::anyParameters());
