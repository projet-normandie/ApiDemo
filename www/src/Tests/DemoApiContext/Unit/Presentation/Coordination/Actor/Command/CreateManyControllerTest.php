...
use DemoApiContext\Presentation\Coordination\Actor\Command\CreateManyController;
use Phake;
use Phake_IMock;
...
/**
 * Class CreateManyControllerTest
 *
 * @category Tests\DemoApiContext\Unit
 * @package Presentation
 * @subpackage Coordination\Actor\Command
 *
 * @coversDefaultClass CreateManyController
 * @group unit
 * @requires PHP 7.0
 *
 * @license MIT
 */
class CreateManyControllerTest extends TestCase
{
    /**
    ...
    /** @var CreateManyCommand $controller */
    ...
    public function setUp(): void
    {
        ...
        $this->controller = new CreateManyController(
            $this->commandAdapter,
            $this->commandRequest,
            $this->commandHandler,
    ...
    {
        $createManyCommand = Phake::mock(CreateManyCommand::class);
        Phake::when($this->commandAdapter)
            ->buildCommandFromRequest(Phake::anyParameters())
            ->thenReturn($createManyCommand);

        Phake::when($this->commandHandler)
            ->process($createManyCommand)
            ->thenReturn([]);
